...
const modalContent = (modal, closeModal) => (
	<div className="modal">
		<div className={`title ${modal.type}`}>
			{modal.title} <Button onClick={() => closeModal(modal.id)}>X</Button>
		</div>
		{modal &&
			((modal.type == CUSTOM_MODAL && modal.custom) || (
				<div className="description">{modal.text}</div>
			))}
		{modal && modal.type != CUSTOM_MODAL && (
...
export const Modal = () => {
	...
	const modalOK =
		(modal && modal.custom && modal.custom.$$typeof) ||
		(modal && !modal.custom);
	if (!modalOK) {
		closeModal();
	}
