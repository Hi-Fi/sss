...
const ControlledCheckbox = ({ name, label, control }) => {
	const labelId = `${name}-label`;
	return (
		<section id={name}>
			...
			<div>
				...
				<div>
					<Controller
						name="event"
						render={({ field }) => (
							<TextField label="Event" variant="standard" {...field} />
						)}
						control={control}
						defaultValue=""
					/>
				...
				<div>
					<ControlledSelect name="fontSize" label="Font size" control={control}>
						...
						{useCoverImage && (
							<div>
								<ControlledFileUpload
									...
									currentImage={{
										name: leafletInfo.coverImageName,
										data: leafletInfo.coverImage,
									}}
								/>
							</div>
						)}
