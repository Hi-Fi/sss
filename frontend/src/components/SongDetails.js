				...
				<div style={authorMelodyStyle}>
					{song.arrangers && (
						<span style={authorStyle}>
							Arranger: {this.renderAuthors(song.arrangers)}
						</span>
					)}
					{song.composers && (
						<span style={authorStyle}>
							Composer: {this.renderAuthors(song.composers)}
						</span>
					)}
					{song.lyricists && (
						<span style={authorStyle}>
							Lyricist: {this.renderAuthors(song.lyricists)}
						</span>
					)}
				</div>
				<ol>{this.renderVerses(song.verses)}</ol>
				<div style={extraStyle}>{song.extraInfo}</div>
				{song.tags && (
					<div id="tags">
						<h2>Tags</h2>
						<div className="tags">{this.renderTags(song.tags)}</div>
					</div>
				)}
			</div>
		);
