...
export const Tabs = () => {
	...
	return (
		<div>
						...
						<MuiTab
							...
							label={
								<TabLabel
									name={singleTab.name}
									id={singleTab.id}
									closeFunction={() => closeTab(singleTab.id)}
								/>
							}
							value={"/song/" + singleTab.id}
						/>
					))}
					;
				</MuiTabs>
			...
			{value.startsWith("/leaflet") && (
				<Tab>
					<LeafletTabsContainer leaftletTabPath={value} />
				</Tab>
			)}
			...
			{openTabs.map(
				(singleTab) =>
					value === "/song/" + singleTab.id && (
						<Tab key={singleTab.id}>
							<SongsShow id={singleTab.id} />
						</Tab>
					),
			)}
		</div>
