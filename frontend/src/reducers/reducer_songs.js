...
function songsReducer(state = INITIAL_STATE, action) {
  ...
  switch (action.type) {
    ...
    case FETCH_SONGS_SUCCESS: {
      ...
      return { ...state, songsList: { songs: songs, error: null, loading: false } };
    }
    ...
    case SELECT_SONG: {
      ...
      let selectedSong = newSongList.songs.find(song => song.id === action.song.id)
    ...
    case SELECT_ALL_SONGS: {
      ...
      return { ...state, songsList: { ...state.songsList, songs: newSongList }, allSelected: selected }
    }
    default:
      return state;
  }
