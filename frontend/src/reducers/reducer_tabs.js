...
const tabReducer = (state = INITIAL_STATE, action) => {
	switch (action.type) {
		case CHANGE_TAB: {
			...
			if (
				action.id.includes("/song/") &&
				state.songTabs.tabs.filter(function (tab) {
					return newTab.includes(tab.id);
				}).length === 0
			) {
				newTab = "/";
			}
		...
		case ADD_TAB:
			if (
				...
				return {
					...state,
					openTab: "/song/" + action.id,
					songTabs: {
						tabs: [
							...state.songTabs.tabs,
							{ id: action.id, name: "Loading..." },
						],
					},
				};
			}
		case CLOSE_TAB:
			return {
				...
				openTab: "/",
				songTabs: {
					tabs: state.songTabs.tabs.filter(function (tab) {
						return tab.id !== action.id;
					}),
				},
			};
