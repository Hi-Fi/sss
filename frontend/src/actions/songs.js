...
import {
	selectAll as leafletSelectAll,
	deselectAll as leafletDeselectAll,
...
export async function fetchAllSongs(dispatch, getState) {
	...
	if (songs.length > 0) {
		...
		fetcher(`${ROOT_URL}/api/v1/songs`, {
			...
			.then(async (response) => {
				!response.error
					? dispatch(fetchSongsSuccess(await response.json()))
					: dispatch(fetchSongsFailure(response.statusText));
			})
...
export function fetchSong(id) {
	return async function fetchSong(dispatch, getState) {
		...
		if (song !== undefined) {
			...
			fetcher(`${ROOT_URL}/api/v1/song/${id}`)
				.then(async (response) => {
				...
				.catch((error) => {
					...
					// dispatch(closeTab(id))
					dispatch(
						openModal(ERROR_MODAL, "Couldn't load song", error.message, id),
					);
				});
		}
	};
